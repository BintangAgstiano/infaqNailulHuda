...
@section('content')
    ...
    <div class="card custom-card">
        <div class="card-header justify-content-between">
            <div class="card-title">Edit Pemasukan</div>
            <a href="{{ url('pemasukanview') }}" class="btn btn-primary btn-wave">Table Pemasukan</a>
        </div>
        <div class="card-body">
            <form action="{{route('updatePemasukan',$data->id)}}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="nominal" class="form-label fs-14 text-dark">Nominal</label>
                    <input type="text" value="{{'Rp. '.number_format($data->nominal, 0 ,",", ".")}}" class="rupiahs form-control showNom" id="nominal" placeholder="" name="nominal"
                        value="{{ old('nominal') }}">
                    @error('nominal')
